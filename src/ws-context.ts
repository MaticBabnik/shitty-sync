...
export function wsCtx(roomMap:Map<string,Room>, io:Socket) {
    return (socket:Socket, next:(error?:any)=>void)=>{
        ...
        if (!roomId) {
            ...
            console.log('[ws-ctx] invalid room');
            ...
            if (!roomMap.get(roomId)) {
                ...
                console.log(`[ws-ctx] User ${socket.id} created room ${roomId}`);
            }else{
                roomMap.get(roomId)?.users.set(socket.id,new User(socket.id));
                console.log(`[ws-ctx] Socket ${socket.id} joined room ${roomId}`);
            }
            socket.join(roomId); //this has to be in else, otherwise TS is mad


            socket.onAny((eventName:string)=>{
                console.log(`[ws-ctx] socket: ${socket.id} event: ${eventName}`);
                
                let rooms = Array.from(socket.rooms).filter(x=>x !== socket.id);
                socket.ctx = roomMap.get(rooms[0])
                console.log(`[ws-ctx] set socket.ctx to room ${roomId}`);
            })
            ...
            socket.on('disconnect',()=>{
                ...
                console.log(`[ws-ctx] User ${socket.id} left`);
                if (room.users.size === 0){
                    console.log(`[ws-ctx] Room ${roomId} is empty, deleting`);
