...
import { Room, RoomManager, MediaType } from './sync';

export function expressMetrics(app: express.Application, registry: prometheusClient.Registry) {
...
export function roomMetrics(roomManager: RoomManager, registry: prometheusClient.Registry) {
    ...
    roomManager.on('roomsourcechanged',r=>{
        if (r.media.type==='offline') return;
